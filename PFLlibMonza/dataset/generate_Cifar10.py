#!/usr/bin/env python3
"""Generate MONZA CIFAR10 client partitions.

The MONZA runtime reads ../dataset/Cifar10/{train,test}/{client_id}.npz with a
...
def load_cifar10(root: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    y = np.asarray(train.targets + test.targets, dtype=np.int64)
    return x, y
