...
SWEEP_DIR="${SWEEP_DIR:-$ROOT/analysis_outputs/threshold_sweep}"
TAIL_ROUNDS="${TAIL_ROUNDS:-30}"
RUN_LOG="${RUN_LOG:-$ROOT/threshold_sweep_$(date +%Y%m%d_%H%M%S).log}"
...
run_monza_threshold() {
  ...
  log "Run ${detector} cc=${cc} threshold=${threshold}"
  rm -rf "$candidate_dir"
  ...
  local h5_file
  h5_file="$(find "$ROOT/PFLlibMonza/results" -maxdepth 1 -name "MNIST_FedAvg_${cc}_*_test_0.h5" -printf '%T@ %p\n' | sort -n | tail -1 | cut -d' ' -f2-)"
  if [[ -n "$h5_file" ]]; then
...
main() {
  ...
  log "Summarize sweep"
  "$VENV_PY" scripts/summarize_threshold_sweep.py \
    --sweep-dir "$SWEEP_DIR" \
